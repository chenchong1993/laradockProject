...
<script>
  ...
  function init() {
    ...
    require([
        ...
        //初始化F2楼层平面图
        var f1 = new DynamicMapServiceLayer("http://121.28.24.203:5567/arcgis/rest/services/airport/airport/MapServer");
        map.addLayer(f1);

        //初始化路网
        // var f2 = new DynamicMapServiceLayer("http://121.28.24.203:5567/arcgis/rest/services/C7/network1/MapServer");
        // map.addLayer(f2);
                    ...
                    if (dat.status == 0) {
                        ...
                        for (var i in dat.data.users) {
                            ...
                            if(dat.data.users[i].floor == 2){
                            	console.log(dat.data.users[i].name);
                                //打点
                                addUserPoint(
                                    dat.data.users[i].id,
                                    dat.data.users[i].y,
