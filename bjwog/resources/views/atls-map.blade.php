...
<div class="row" style="height: 100%">
    ...
    <div id="map_atls_1" class="col-md-12">
        <h5 style="position: absolute; z-index: 10">一层</h5>
    </div>
  	...
  	<div style="position: absolute; z-index: 10; top: 10px; right: 30px";>
            ...
            <button class="btn btn-default" onclick="sendSingleMsg()">发送消息测试</button> 
    </div>
  ...
  function init() {
    ...
    */
                    ...
                    if (dat.status == 0) {
                        ...
                        for (var i in dat.data.users) {
                            ...
                            $('#tab-user-list').append(
                                ...
                                '<td><button class="btn btn-default btn-xs" onclick="locationTo('+ dat.data.users[i].x +','+ dat.data.users[i].y + ',' + dat.data.users[i].floor + ')">查看</button></td>'+
                                '</tr>'
                                );
                        }
                        notify("查找成功", "sys");
                    ...
                    if (dat.status == 0) {
                        ...
                        for (var i in dat.data.devices) {
                            ...
                            $('#tab-device-list').append(
                                ...
                                '<td><button class="btn btn-default btn-xs" onclick="locationTo('+ dat.data.devices[i].x +','+ dat.data.devices[i].y + ',' + dat.data.devices[i].floor + ')">查看</button></td>'+
                                '</tr>'
                                );
                        }
                        notify("查找成功", "sys");
    ...
    require([
        ...
        //初始化F1楼层平面图
        var f1 = new DynamicMapServiceLayer("http://121.28.24.203:5567/arcgis/rest/services/outlets/outlets1f/MapServer");
        // var f1 = new DynamicMapServiceLayer("http://121.28.24.203:5567/arcgis/rest/services/outlets/outlets2f/MapServer");
        map1.addLayer(f1);
